...
void PXL_Bitmap::fill(PXL_Colour colour) {
	...
	for (int y = 0; y < height; ++y) {
		...
		for (int x = 0; x < row_size; x += channel.num_channels) {
			if (channel.num_channels >= 1) pixels[x + row_y] = r;
			if (channel.num_channels >= 2) pixels[x + 1 + row_y] = g;
			if (channel.num_channels >= 3) pixels[x + 2 + row_y] = b;
			if (channel.num_channels >= 4) pixels[x + 3 + row_y] = a;
		}
	}
}
...
bool PXL_Bitmap::check_has_transparency() {
	...
	if (pixels == NULL) {
		...
		for (int n = 0; n < buffer_size; n += channel.num_channels) {
			if (pixels[n + (channel.num_channels - 1)] == 0) {
